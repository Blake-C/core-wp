...
if ( ! function_exists( 'core_wp_posted_on' ) ) {
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function core_wp_posted_on() {
		// Posted on.
		...
		/* translators: The post publish date */
			esc_html_x( 'Posted: %s', 'post date', 'core_wp' ),
			$time_string
		);
		...
		if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
			...
			/* translators: The post updated date */
				esc_html_x( 'Updated: %s', 'updated date', 'core_wp' ),
				$time_string_updated
			);
		...
		/* translators: The post author, byline */
			esc_html_x( 'by %s', 'post author', 'core_wp' ),
