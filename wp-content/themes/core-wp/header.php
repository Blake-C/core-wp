...
<head>
	...
	<?php core_wp_search_results_meta_description(); ?>

	<?php wp_head(); ?>
</head>

<body <?php body_class(); ?>>
	<a class="skip-link show-for-sr" href="#content"><?php echo esc_html_x( 'Skip to content', 'Link to jump to div with main content', 'core_wp' ); ?></a>
	...
	<?php if ( is_super_admin() && CORE_WP_INLINE_DEBUG ) : ?>
		<a href="#" id="theme_debug_regions"><?php echo esc_html_x( 'Show Regions', 'Link to display inline debug for theme files', 'core_wp' ); ?></a>
	<?php endif; ?>
	...
	<?php if ( has_nav_menu( 'primary' ) ) : ?>
		...
		<?php
		$core_wp_blog_name = get_bloginfo( 'name' ) ? '<li class="menu-text">' . get_bloginfo( 'name' ) . '</li>' : '';
		...
		*/
		$core_wp_menu_args = array(
			...
			'link_before'     => '',
			'link_after'      => '',
			'items_wrap'      => '<ul id="%1$s" class="%2$s">' . $core_wp_blog_name . '%3$s</ul>',
			'depth'           => 0,
			'walker'          => '',
		);

		wp_nav_menu( $core_wp_menu_args );
		?>
		</div>
	<?php endif; ?>
